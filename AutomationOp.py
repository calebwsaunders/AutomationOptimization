...
print('Do you want to combine new prescriptions and refills, or separate new fills from refills?\n')
...
# 5 = minor controls
# 6 = CIIs
print('Which class of medications are you optimizing?\n')
...
# If user chooses to combine new and refill prescriptions:
if new_refill_request == 'Yes':
    ...
    sorted_occurences = sorted(pertinent_occurrences, key=get_item_id)
    ...
    sheet.oddHeader.left.text = f'Automation Optimization: {month_abbr}{data_year}'
    sheet.oddHeader.left.size = 16
...
# If user chooses to separate new prescriptions from refills:
else:
    # Setting up a loop to iterate over every excel file identified above.
    for file in files:
        wb = openpyxl.load_workbook(file)
    ...
    # Excluding medications and supplies that cannot be placed in automation, then adding pertinent
    # items to a new list.
    pertinent_new_occurrences = []
    pertinent_refill_occurrences = []
    ...
    # Formatting and writing to the header.
    sheet_new.oddHeader.left.text = f'Automation Optimization: {month_abbr}{data_year}'
    sheet_new.oddHeader.left.size = 16
    sheet_new.oddHeader.left.font = 'Times New Roman'

    sheet_refills.oddHeader.left.text = f'Automation Optimization: {month_abbr}{data_year}'
    sheet_refills.oddHeader.left.size = 16
    sheet_refills.oddHeader.left.font = 'Times New Roman'
